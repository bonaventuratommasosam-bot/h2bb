...
const REASON = {
  ...
  AI_VETO: 'ai_veto_entry',
  AI_EXIT: 'ai_exit',
  AI_THRESHOLD: 'ai_threshold_adjust',
  UNKNOWN: 'unknown',
};
...
function inferReasonCode({ action, reason, bias, riskReasons }) {
  ...
  if (r.includes('ai-veto') || r.includes('ai veto') || r.includes('ai:')) return REASON.AI_VETO;
  if (r.includes('ai-exit') || r.includes('ai exit')) return REASON.AI_EXIT;
