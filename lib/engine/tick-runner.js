  ...
  try {
    ...
    try {
      ...
      if (mcResult.changed) {
        ...
        // no Telegram for meta — entry/exit only
      }
    } catch (e) {
...
async function _runTickInternal() {
  ...
  // Flat: clear absolute TP/SL leftovers so they never freeze entries
  if (!hasPosition && (shared.strategy.stopLoss != null || shared.strategy.takeProfit != null)) {
    shared.strategy.stopLoss = null;
    shared.strategy.takeProfit = null;
    try { require('../state/strategy').saveStrategy(); } catch { /* ignore */ }
  }
  // Absolute SL/TP only with an open position
  if (hasPosition && shared.strategy.stopLoss && price < shared.strategy.stopLoss) {
  ...
  if (hasPosition && shared.strategy.takeProfit && price > shared.strategy.takeProfit) {
