...
function applyHardCaps(strategy) {
  ...
  if (strategy.minConfidenceScore != null) {
    // Super degen may run below the default floor (operator intentional YOLO)
    const mode = String(strategy.aiMode || '').toLowerCase();
    const superDegen = mode === 'super_degen' || mode === 'mega_super_degen' || mode === 'yolo';
    const scoreFloor = superDegen
      ? Math.min(HARD_CAPS.minConfidenceScoreMin, HARD_FLOORS.minConfidenceScore)
